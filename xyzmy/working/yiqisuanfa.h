...
//         }
    ...
    */
    ...
    //波峰判断
    if ((peak[1].max!=0) && (peak[0].max<peak[1].max))  //存在T峰、C峰
    {
        feng[0] = peak[0].start;  //第一个波峰
        feng[1] = peak[0].end;
        feng_max[0] = peak[0].max;

        feng[2] = peak[1].start;  //第二个波峰
        feng[3] = peak[1].end;
        feng_max[1] = peak[1].max;
    }
    else if((peak[1].max!=0) && (peak[0].max>=peak[1].max))
    {
        feng[0] = peak[1].start;  //第一个波峰
        feng[1] = peak[1].end;
        feng_max[0] = peak[1].max;

        feng[2] = peak[0].start;  //第二个波峰
        feng[3] = peak[0].end;
        feng_max[1] = peak[0].max;
    }
    else if((peak[1].max==0) && (peak[0].max>0 && peak[0].max<600))  //只存在T峰
    {
        feng[0] = peak[0].start;  //第一个波峰
        feng[1] = peak[0].end;
        feng_max[0] = peak[0].max;
    }
    else if((peak[1].max==0) && (peak[0].max>600 && peak[0].max<1100))  //只存在C峰
    {
        feng[2] = peak[0].start;  //第二个波峰
        feng[3] = peak[0].end;
        feng_max[1] = peak[0].max;
